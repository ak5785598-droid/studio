...
const STORE_ITEMS = [
  { id: 'f7', name: 'Celestial Wings', type: 'Frame', price: 15000, description: 'Tiered lavender wings with sovereign golden peaks and crystalline blue rings.', icon: Sparkles, color: 'text-indigo-400' },
                  ...
                  return (
                    <Card key={item.id} className="relative overflow-hidden group border-none shadow-lg rounded-[2.5rem] bg-white hover:scale-105 transition-all duration-300">
                      <div className={cn(
                        ...
                        (item.id === 'f5' || item.id === 'f6' || item.id === 'f7') && "bg-gradient-to-br from-indigo-50 via-white to-indigo-50"
                      )}>
                        {item.type === 'Frame' ? (
                        ...
                        <Badge className="absolute top-6 right-6 bg-secondary/80 text-foreground border-none font-black uppercase text-[10px] tracking-widest px-3">
                          {item.id === 'f5' ? 'Elite' : item.id === 'f7' ? 'Celestial' : item.type}
                        </Badge>
                      </div>
