...
export default function FruitSlotsPage() {
  ...
  const startSpin = () => {
    ...
    let currentStep = 0;
    const totalSteps = 24 + targetIdx; // At least 3 full loops
    let speed = 60;
  ...
  return (
    <AppLayout fullScreen>
      <div className="h-[100dvh] w-full bg-[#311b92] flex flex-col relative overflow-hidden font-headline text-white select-none">
        ...
        {/* Side Progress Gradients */}
        <div className="absolute left-2 top-44 bottom-60 w-6 flex flex-col gap-1 p-1 bg-black/40 rounded-full border border-white/5 z-50">
           {Array.from({length: 12}).map((_, i) => (
             <div key={i} className={cn("flex-1 rounded-sm", i < 4 ? "bg-cyan-400" : i < 8 ? "bg-blue-500" : "bg-blue-700")} />
           ))}
        </div>
        <div className="absolute right-2 top-44 bottom-60 w-6 flex flex-col gap-1 p-1 bg-black/40 rounded-full border border-white/5 z-50">
           {Array.from({length: 12}).map((_, i) => (
             <div key={i} className={cn("flex-1 rounded-sm", i < 4 ? "bg-pink-400" : i < 8 ? "bg-purple-500" : "bg-purple-700")} />
           ))}
        </div>

        {/* Main Golden Machine Body */}
